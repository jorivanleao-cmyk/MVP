...
function isAdminMessage(text: string) {
  return text.startsWith('/assumir') || text.startsWith('/liberar') || text.startsWith('/status');
}

async function handleAdminMessage(conversationId: number, phone: string, text: string) {
  if (text.startsWith('/assumir')) {
    await prisma.conversation.update({ where: { id: conversationId }, data: { humanTakeover: true, status: ConversationStatuses.HUMAN_TAKEOVER } });
    return 'Atendimento humano ativado. Aguarde um atendente humano.';
  }

  if (text.startsWith('/liberar')) {
    await prisma.conversation.update({ where: { id: conversationId }, data: { humanTakeover: false, status: ConversationStatuses.ACTIVE } });
    return 'Atendimento humano liberado. A IA retorna a ajudar.';
  }

  if (text.startsWith('/status')) {
    const conversation = await prisma.conversation.findUnique({ where: { id: conversationId } });
    return conversation ? `Status atual: ${conversation.status}. humanTakeover=${conversation.humanTakeover}` : 'Conversa não encontrada.';
  }
...
export async function processIncomingMessage(payload: WhatsAppPayload) {
  ...
  await prisma.message.create({
    data: { conversationId: conversation.id, from: MessageFromValues.CLIENT, content: text },
  });
  ...
  const analysis = await analyzeMessage({ text, phone, conversationId: conversation.id, clientId: client.id });

  await prisma.message.create({
    data: { conversationId: conversation.id, from: MessageFromValues.AI, content: analysis.reply },
  });

  await prisma.conversation.update({
    where: { id: conversation.id },
    data: {
      status: analysis.intent === 'agendar' ? ConversationStatuses.WAITING_CONFIRMATION : conversation.status,
